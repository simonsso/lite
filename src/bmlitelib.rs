...
use tests::embedded_hal_mock::gpio::*;
    ...
    fn enroll_test_case() {
		use super::*;
        let expectations = [
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x02,0x40,0x01,0x00,0x02,0x20,0x00,0x00,0x6A,0x6F,0xB3,0xC9,].to_vec(),
                    [0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x17,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x11,0x00,0x01,0x00,0x01,0x00,0x02,0x40,0x02,0x00,0x02,0x20,0x04,0x00,0x08,0x00,0x00,0x00,0x01,0x20,0x01,0x00,0x00,0x26,0xCF,0xA1,0x1D,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x26,0x26,0x26,0x26,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x01,0x00,0x03,0x00,0x00,0x00,0x96,0x3B,0x49,0x0D,].to_vec(),
                    [0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,0x26,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x09,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x01,0x00,0x01,0x20,0x01,0x00,0x00,0x46,0xDD,0xA8,0xA9,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x46,0x46,0x46,0x46,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x07,0x00,0x01,0x00,0x02,0x00,0x00,0x00,0x97,0x52,0x15,0xFD,].to_vec(),
                    [0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x09,0x00,0x01,0x00,0x01,0x00,0x07,0x00,0x01,0x00,0x01,0x20,0x01,0x00,0x00,0x09,0x98,0x3F,0xE3,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x09,0x09,0x09,0x09,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0A,0x00,0x04,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x00,0x00,0x52,0x7C,0x2B,0x55,].to_vec(),
                    [0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x09,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x20,0x01,0x00,0x00,0x83,0xE1,0x25,0x90,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x83,0x83,0x83,0x83,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x01,0x00,0x04,0x00,0x00,0x00,0x2F,0x03,0x9E,0x90,].to_vec(),
                    [0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,0x83,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x1C,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x16,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x03,0x00,0x02,0x20,0x04,0x00,0x02,0x00,0x00,0x00,0x02,0x60,0x01,0x00,0x02,0x01,0x20,0x01,0x00,0x00,0x6B,0x22,0x1E,0x22,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x6B,0x6B,0x6B,0x6B,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x07,0x00,0x01,0x00,0x02,0x00,0x00,0x00,0x97,0x52,0x15,0xFD,].to_vec(),
                    [0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,0x6B,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                ...
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x1C,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x16,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x03,0x00,0x02,0x20,0x04,0x00,0x01,0x00,0x00,0x00,0x02,0x60,0x01,0x00,0x02,0x01,0x20,0x01,0x00,0x00,0xB0,0x07,0x7F,0x5E,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0xB0,0xB0,0xB0,0xB0,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x07,0x00,0x01,0x00,0x02,0x00,0x00,0x00,0x97,0x52,0x15,0xFD,].to_vec(),
                    [0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,0xB0,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                ...
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x1C,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x16,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x03,0x00,0x02,0x20,0x04,0x00,0x00,0x00,0x00,0x00,0x02,0x60,0x01,0x00,0x01,0x01,0x20,0x01,0x00,0x00,0x68,0x94,0xE4,0x45,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x68,0x68,0x68,0x68,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x0E,0x00,0x08,0x00,0x01,0x00,0x01,0x00,0x02,0x00,0x01,0x00,0x05,0x00,0x00,0x00,0x4A,0x64,0x22,0x28,].to_vec(),
                    [0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,0x68,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                ...
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
                    [0x46,0x46,0x46,0x46,].to_vec(),),
                SpiTransaction::transfer([0x01,0x00,0x14,0x00,0x0E,0x00,0x01,0x00,0x01,0x00,0x06,0x00,0x02,0x00,0x08,0x10,0x00,0x00,0x06,0x00,0x02,0x00,0x09,0x00,0xE9,0xB0,0x6B,0x51,].to_vec(),
                    [0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,0x46,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x7F,0xFF,0x01,0x7F,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,].to_vec(),
                    [0x00,0x00,0x0F,0x00,].to_vec(),),
                SpiTransaction::transfer([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,].to_vec(),
                    [0x09,0x00,0x01,0x00,0x01,0x00,0x06,0x00,0x01,0x00,0x01,0x20,0x01,0x00,0x00,0x4A,0x8C,0x44,0xF4,].to_vec(),),
                SpiTransaction::transfer([0x7F,0xFF,0x01,0x7F,].to_vec(),
		...
		let mut bm = BmLite::new(spi, dummy_cs,dummy_reset,dummy_irq );
        let mut callback_cnt=0;
		let ans = bm.enroll(|_|{callback_cnt += 1;});
        match ans {
            Err(_x) => {assert!(false, "Function returned unexpected error")}
            Ok(ans) => {assert!(ans == 0) }
        }
